...
{%
	...
	\node[fill=thmtitle@bg@color,text=thmtitle@color,below left,draw=thmtitle@bg@color,very thick,text width={\dimexpr\marginleft-2mm},align=center] at ($(texte.north west)+(1pt,0)$) {\thmtitle@font \titletext\pluriel@thm \ifnum\thebreak@box>0\par(suite)\fi};
	\end{tikzpicture}
}
...
{%
	...
	\typebox{dem}
	\def\titletext{\Huge \kai 证明:}
	\ifx#1\@empty\xdef\indic{0}\else\xdef\indic{#1}\fi
	\xdef\indic{#1}
	\begin{lrbox}{\dem@box}
	\begin{minipage}{\dimexpr\linewidth-0.5\marginleft}
	{\demtitle@font \titletext}\par\medskip\kai%
}
{%
...
{%
	...
	\node[right,text=activitytitle@color] at (0.75,0) {\hei #1};
	\end{tikzpicture}
...
\newcommand{\exercises}[1][0]
{%
	\newpage
	\pagecolor{exercices@bg@color}
	\begin{flushright}
	\begin{tikzpicture}
	\node[inner sep=1em] (titre) {\exercicetitle@font 习题};
	...
	\node[inner sep=1em,text=exercicetitle@color] at (titre) {\exercicetitle@font 习题};
	\end{tikzpicture}
	\end{flushright}
}
...
{%
	...
	% \textcolor{corref@color}{\corref@font \hyperlink{\thechapter-\theexercice}{答案}}
...
{%
	...
	\begin{flushright}
	\begin{tikzpicture}
	\node[inner sep=1em] (titre) {\corrigetitle@font 答案};
	\fill[rounded corners=2pt,fill=shadow@color] ($(titre.north west)+(2pt,-2pt)$) -- ($(titre.north east)+(1mm,0mm)+(2pt,-2pt)$) -- ($(titre.south east)+(2pt,-2pt)$) -- ($(titre.south west)+(-1mm,0)+(2pt,-2pt)$) -- cycle;
	...
	\node[inner sep=1em,text=corrigetitle@color] at (titre) {\corrigetitle@font 答案};
	\end{tikzpicture}
	\end{flushright}
}
...
\newenvironment{ans}[1][0]
{%
	\ifnum#1=0
		\refstepcounter{answer}
		\expandafter\xdef\csname nbexos\thechapter\endcsname{\theanswer}
		\ifnum\theanswer<10
			\def\num@exo{0\theanswer}
		\else
			\def\num@exo{\theanswer}
...
\renewcommand{\contentsname}{Physics is fun!}
